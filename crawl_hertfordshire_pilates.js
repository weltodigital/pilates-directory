...
/**
 * Crawl Google Places API to find real pilates studios in Hertfordshire
 * Uses the same successful approach as Hampshire and Herefordshire
 */
...
// Hertfordshire cities with coordinates for radius searches
const HERTFORDSHIRE_CITIES = [
  { name: 'Cheshunt', lat: 51.7026, lng: -0.0316 },
  { name: 'Hatfield', lat: 51.7632, lng: -0.2312 },
  { name: 'Hemel Hempstead', lat: 51.7545, lng: -0.4680 },
  { name: 'Hitchin', lat: 51.9492, lng: -0.2883 },
  { name: 'Letchworth', lat: 51.9781, lng: -0.2281 },
  { name: 'Rickmansworth', lat: 51.6422, lng: -0.4736 },
  { name: 'St Albans', lat: 51.7500, lng: -0.3375 },
  { name: 'Stevenage', lat: 51.9020, lng: -0.2019 },
  { name: 'Watford', lat: 51.6558, lng: -0.3967 },
  { name: 'Welwyn Garden City', lat: 51.8008, lng: -0.2035 }
];
...
async function searchPilatesStudiosInCity(city) {
  ...
  const radius = 10000; // 10km radius for suburban areas
          ...
          if (place.name.toLowerCase().includes('pilates') ||
            ...
            try {
              ...
              if (detailsData.result) {
                ...
                // Filter for Hertfordshire addresses
                if (details.formatted_address &&
                    (details.formatted_address.includes('Hertfordshire') ||
                     details.formatted_address.includes(city.name) ||
                     isInHertfordshire(details.formatted_address))) {
...
function isInHertfordshire(address) {
  const hertfordshireIndicators = [
    'Hertfordshire', 'Herts', 'AL1', 'AL2', 'AL3', 'AL4', 'AL5', 'AL6', 'AL7', 'AL8', 'AL9', 'AL10',
    'HP1', 'HP2', 'HP3', 'HP4', 'SG1', 'SG2', 'SG3', 'SG4', 'SG5', 'SG6', 'SG7', 'SG8', 'SG9', 'SG10',
    'WD3', 'WD4', 'WD5', 'WD17', 'WD18', 'WD19', 'WD23', 'WD24', 'WD25',
    'EN6', 'EN7', 'EN8', 'EN10', 'EN11',
    'Cheshunt', 'Hatfield', 'Hemel Hempstead', 'Hitchin', 'Letchworth', 'Rickmansworth', 'St Albans', 'Stevenage', 'Watford', 'Welwyn Garden City'
  ];

  return hertfordshireIndicators.some(indicator =>
...
function determineCityFromAddress(address, searchCity) {
  ...
  // Check for specific Hertfordshire cities
  const cityNames = HERTFORDSHIRE_CITIES.map(c => c.name.toLowerCase());
  for (const cityName of cityNames) {
    if (addressLower.includes(cityName)) {
      return HERTFORDSHIRE_CITIES.find(c => c.name.toLowerCase() === cityName).name;
    }
  }

  // Check for Welwyn Garden City variations
  if (addressLower.includes('welwyn garden city') || addressLower.includes('welwyn')) {
    return 'Welwyn Garden City';
  }
...
async function deleteExistingHertfordshireStudios() {
  console.log('🗑️ Clearing existing Hertfordshire studios...');

  const { error } = await supabase
    .from('pilates_studios')
    .delete()
    .eq('county_slug', 'hertfordshire');

  if (error) {
    throw new Error(`Failed to delete existing studios: ${error.message}`);
  }

  console.log('✅ Cleared existing Hertfordshire studios');
}
...
async function insertCrawledStudios(studios) {
  ...
  const studiesToInsert = studios.map(studio => {
    ...
    return {
      ...
      city_slug: citySlug,
      full_url_path: `hertfordshire/${citySlug}/${slug}`,
      ...
      description: `Professional pilates studio in ${city}, Hertfordshire. Offering expert instruction with modern equipment and a welcoming environment for all levels.`,
  ...
  const { error: updateError } = await supabase
    ...
    .eq('slug', 'hertfordshire')
    .eq('type', 'county');

  if (updateError) {
    console.log(`⚠️ Warning: Could not update county count: ${updateError.message}`);
  } else {
    console.log(`✅ Updated Hertfordshire county studio count to ${studiesToInsert.length}`);
  }
...
async function main() {
  console.log('🚀 Crawling Google Places API for real Hertfordshire pilates studios...\n');

  try {
    let allStudios = [];

    // Search each Hertfordshire city
    for (const city of HERTFORDSHIRE_CITIES) {
    ...
    await deleteExistingHertfordshireStudios();
    const insertedCount = await insertCrawledStudios(uniqueStudios);

    console.log('\n' + '='.repeat(60));
    console.log('🎉 HERTFORDSHIRE GOOGLE PLACES CRAWL COMPLETE');
    ...
    console.log('🏆 Hertfordshire now has 100% real, Google-verified pilates studios!');
