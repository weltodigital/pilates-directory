...
/**
 * Crawl Google Places API to find real pilates studios in Lancashire
 * Uses the same successful approach as Hampshire, Herefordshire, Hertfordshire, Isle of Wight, and Kent
 */
...
// Lancashire cities with coordinates for radius searches
const LANCASHIRE_CITIES = [
  { name: 'Accrington', lat: 53.7535, lng: -2.3608 },
  { name: 'Blackburn', lat: 53.7480, lng: -2.4820 },
  { name: 'Blackpool', lat: 53.8176, lng: -3.0359 },
  { name: 'Burnley', lat: 53.7888, lng: -2.2421 },
  { name: 'Chorley', lat: 53.6532, lng: -2.6305 },
  { name: 'Fleetwood', lat: 53.9248, lng: -3.0108 },
  { name: 'Lancaster', lat: 54.0466, lng: -2.8007 },
  { name: 'Lytham St Annes', lat: 53.7367, lng: -3.0420 },
  { name: 'Ormskirk', lat: 53.5688, lng: -2.8839 },
          ...
          if (place.name.toLowerCase().includes('pilates') ||
            ...
            try {
              ...
              if (detailsData.result) {
                ...
                // Filter for Lancashire addresses
                if (details.formatted_address &&
                    (details.formatted_address.includes('Lancashire') ||
                     details.formatted_address.includes(city.name) ||
                     isInLancashire(details.formatted_address))) {
...
function isInLancashire(address) {
  const lancashireIndicators = [
    'Lancashire', 'Lancs',
    'BB1', 'BB2', 'BB3', 'BB4', 'BB5', 'BB6', 'BB7', 'BB8', 'BB9', 'BB10', 'BB11', 'BB12', 'BB18',
    ...
    'PR0', 'PR1', 'PR2', 'PR3', 'PR4', 'PR5', 'PR6', 'PR7', 'PR8', 'PR9', 'PR25', 'PR26',
    'L39', 'L40',
    'Accrington', 'Blackburn', 'Blackpool', 'Burnley', 'Chorley', 'Fleetwood', 'Lancaster', 'Lytham St Annes', 'Ormskirk', 'Preston'
  ];

  return lancashireIndicators.some(indicator =>
...
function determineCityFromAddress(address, searchCity) {
  ...
  // Check for specific Lancashire cities
  const cityNames = LANCASHIRE_CITIES.map(c => c.name.toLowerCase());
  for (const cityName of cityNames) {
    if (addressLower.includes(cityName)) {
      return LANCASHIRE_CITIES.find(c => c.name.toLowerCase() === cityName).name;
    }
  }

  // Check for Lytham St Annes variations
  if (addressLower.includes('lytham st annes') || addressLower.includes('lytham') || addressLower.includes('st annes')) {
    return 'Lytham St Annes';
  }
...
async function deleteExistingLancashireStudios() {
  console.log('🗑️ Clearing existing Lancashire studios...');

  const { error } = await supabase
    .from('pilates_studios')
    .delete()
    .eq('county_slug', 'lancashire');

  if (error) {
    throw new Error(`Failed to delete existing studios: ${error.message}`);
  }

  console.log('✅ Cleared existing Lancashire studios');
}
...
async function insertCrawledStudios(studios) {
  ...
  const studiesToInsert = studios.map(studio => {
    ...
    return {
      ...
      county_slug: 'lancashire',
      city_slug: citySlug,
      full_url_path: `lancashire/${citySlug}/${slug}`,
      ...
      description: `Professional pilates studio in ${city}, Lancashire. Offering expert instruction with modern equipment and a welcoming environment for all levels.`,
  ...
  if (updateError) {
    ...
    console.log(`✅ Updated Lancashire county studio count to ${studiesToInsert.length}`);
  }
...
async function main() {
  console.log('🚀 Crawling Google Places API for real Lancashire pilates studios...\n');

  try {
    let allStudios = [];

    // Search each Lancashire city
    for (const city of LANCASHIRE_CITIES) {
    ...
    await deleteExistingLancashireStudios();
    const insertedCount = await insertCrawledStudios(uniqueStudios);

    console.log('\n' + '='.repeat(60));
    console.log('🎉 LANCASHIRE GOOGLE PLACES CRAWL COMPLETE');
    ...
    console.log('🏆 Lancashire now has 100% real, Google-verified pilates studios!');
