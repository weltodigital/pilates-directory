...
/**
 * Crawl Google Places API to find real pilates studios in Lincolnshire
 * Uses the same successful approach as Hampshire, Herefordshire, Hertfordshire, Isle of Wight, Kent, Lancashire, and Leicestershire
 */
...
// Lincolnshire cities with coordinates for radius searches
const LINCOLNSHIRE_CITIES = [
  { name: 'Boston', lat: 52.9789, lng: -0.0265 },
  { name: 'Gainsborough', lat: 53.3988, lng: -0.7738 },
  { name: 'Grantham', lat: 52.9153, lng: -0.6417 },
  { name: 'Grimsby', lat: 53.5668, lng: -0.0757 },
  { name: 'Lincoln', lat: 53.2307, lng: -0.5406 },
  { name: 'Scunthorpe', lat: 53.5903, lng: -0.6439 },
  { name: 'Skegness', lat: 53.1434, lng: 0.3367 },
  { name: 'Sleaford', lat: 52.9991, lng: -0.4057 },
  { name: 'Spalding', lat: 52.7875, lng: -0.1504 },
  { name: 'Stamford', lat: 52.6508, lng: -0.4789 }
];
          ...
          if (place.name.toLowerCase().includes('pilates') ||
            ...
            try {
              ...
              if (detailsData.result) {
                ...
                // Filter for Lincolnshire addresses
                if (details.formatted_address &&
                    (details.formatted_address.includes('Lincolnshire') ||
                     details.formatted_address.includes(city.name) ||
                     isInLincolnshire(details.formatted_address))) {
...
function isInLincolnshire(address) {
  const lincolnshireIndicators = [
    'Lincolnshire', 'Lincs',
    'LN1', 'LN2', 'LN3', 'LN4', 'LN5', 'LN6', 'LN7', 'LN8', 'LN9', 'LN10', 'LN11', 'LN12', 'LN13',
    'PE1', 'PE2', 'PE3', 'PE4', 'PE5', 'PE6', 'PE7', 'PE8', 'PE9', 'PE10', 'PE11', 'PE12', 'PE20', 'PE21', 'PE22', 'PE23', 'PE24', 'PE25',
    'NG23', 'NG24', 'NG31', 'NG32', 'NG33', 'NG34',
    'DN15', 'DN16', 'DN17', 'DN20', 'DN21', 'DN36', 'DN37',
    'Boston', 'Gainsborough', 'Grantham', 'Grimsby', 'Lincoln', 'Scunthorpe', 'Skegness', 'Sleaford', 'Spalding', 'Stamford'
  ];

  return lincolnshireIndicators.some(indicator =>
...
function determineCityFromAddress(address, searchCity) {
  ...
  // Check for specific Lincolnshire cities
  const cityNames = LINCOLNSHIRE_CITIES.map(c => c.name.toLowerCase());
  for (const cityName of cityNames) {
    if (addressLower.includes(cityName)) {
      return LINCOLNSHIRE_CITIES.find(c => c.name.toLowerCase() === cityName).name;
    }
  }
...
async function deleteExistingLincolnshireStudios() {
  console.log('🗑️ Clearing existing Lincolnshire studios...');

  const { error } = await supabase
    .from('pilates_studios')
    .delete()
    .eq('county_slug', 'lincolnshire');

  if (error) {
    throw new Error(`Failed to delete existing studios: ${error.message}`);
  }

  console.log('✅ Cleared existing Lincolnshire studios');
}
...
async function insertCrawledStudios(studios) {
  ...
  const studiesToInsert = studios.map(studio => {
    ...
    return {
      ...
      county_slug: 'lincolnshire',
      city_slug: citySlug,
      full_url_path: `lincolnshire/${citySlug}/${slug}`,
      ...
      description: `Professional pilates studio in ${city}, Lincolnshire. Offering expert instruction with modern equipment and a welcoming environment for all levels.`,
  ...
  const { error: updateError } = await supabase
    ...
    .eq('slug', 'lincolnshire')
    .eq('type', 'county');

  if (updateError) {
    console.log(`⚠️ Warning: Could not update county count: ${updateError.message}`);
  } else {
    console.log(`✅ Updated Lincolnshire county studio count to ${studiesToInsert.length}`);
  }
...
async function main() {
  console.log('🚀 Crawling Google Places API for real Lincolnshire pilates studios...\n');

  try {
    let allStudios = [];

    // Search each Lincolnshire city
    for (const city of LINCOLNSHIRE_CITIES) {
    ...
    await deleteExistingLincolnshireStudios();
    const insertedCount = await insertCrawledStudios(uniqueStudios);

    console.log('\n' + '='.repeat(60));
    console.log('🎉 LINCOLNSHIRE GOOGLE PLACES CRAWL COMPLETE');
    ...
    console.log('🏆 Lincolnshire now has 100% real, Google-verified pilates studios!');
