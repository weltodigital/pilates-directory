...
/**
 * Crawl Google Places API to find real pilates studios in Oxfordshire
...
// Oxfordshire cities with coordinates for radius searches
const OXFORDSHIRE_CITIES = [
  { name: 'Abingdon', lat: 51.6710, lng: -1.2826 },
  { name: 'Banbury', lat: 52.0628, lng: -1.3417 },
  { name: 'Bicester', lat: 51.9000, lng: -1.1519 },
  { name: 'Carterton', lat: 51.7645, lng: -1.5870 },
  { name: 'Chipping Norton', lat: 51.9427, lng: -1.5442 },
  { name: 'Didcot', lat: 51.6089, lng: -1.2411 },
  { name: 'Faringdon', lat: 51.6555, lng: -1.5836 },
  { name: 'Kidlington', lat: 51.8178, lng: -1.2915 },
  { name: 'Oxford', lat: 51.7520, lng: -1.2577 },
  { name: 'Witney', lat: 51.7836, lng: -1.4855 }
];
          ...
          if (place.name.toLowerCase().includes('pilates') ||
            ...
            try {
              ...
              if (detailsData.result) {
                ...
                // Filter for Oxfordshire addresses
                if (details.formatted_address &&
                    (details.formatted_address.includes('Oxfordshire') ||
                     details.formatted_address.includes(city.name) ||
                     isInOxfordshire(details.formatted_address))) {
...
function isInOxfordshire(address) {
  const oxfordshireIndicators = [
    'Oxfordshire',
    ...
    'HP18', 'HP19',
    'RG9',
    'Abingdon', 'Banbury', 'Bicester', 'Carterton', 'Chipping Norton', 'Didcot', 'Faringdon', 'Kidlington', 'Oxford', 'Witney'
  ];

  return oxfordshireIndicators.some(indicator =>
...
function determineCityFromAddress(address, searchCity) {
  ...
  // Check for specific Oxfordshire cities
  const cityNames = OXFORDSHIRE_CITIES.map(c => c.name.toLowerCase());
  for (const cityName of cityNames) {
    if (addressLower.includes(cityName)) {
      return OXFORDSHIRE_CITIES.find(c => c.name.toLowerCase() === cityName).name;
    }
  }

  // Check for variations
  if (addressLower.includes('chipping norton')) {
    return 'Chipping Norton';
  }
...
async function deleteExistingOxfordshireStudios() {
  console.log('🗑️ Clearing existing Oxfordshire studios...');

  const { error } = await supabase
    .from('pilates_studios')
    .delete()
    .eq('county_slug', 'oxfordshire');

  if (error) {
    throw new Error(`Failed to delete existing studios: ${error.message}`);
  }

  console.log('✅ Cleared existing Oxfordshire studios');
}
...
async function insertCrawledStudios(studios) {
  ...
  const studiesToInsert = studios.map(studio => {
    ...
    return {
      ...
      county_slug: 'oxfordshire',
      city_slug: citySlug,
      full_url_path: `oxfordshire/${citySlug}/${slug}`,
      ...
      description: `Professional pilates studio in ${city}, Oxfordshire. Offering expert instruction with modern equipment and a welcoming environment for all levels.`,
  ...
  if (updateError) {
    ...
    console.log(`✅ Updated Oxfordshire county studio count to ${studiesToInsert.length}`);
  }
...
async function main() {
  console.log('🚀 Crawling Google Places API for real Oxfordshire pilates studios...\n');

  try {
    let allStudios = [];

    // Search each Oxfordshire city
    for (const city of OXFORDSHIRE_CITIES) {
    ...
    await deleteExistingOxfordshireStudios();
    const insertedCount = await insertCrawledStudios(uniqueStudios);

    console.log('\n' + '='.repeat(60));
    console.log('🎉 OXFORDSHIRE GOOGLE PLACES CRAWL COMPLETE');
    ...
    console.log('🏆 Oxfordshire now has 100% real, Google-verified pilates studios!');
