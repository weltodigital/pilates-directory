...
/**
 * Crawl Google Places API to find real pilates studios in South Yorkshire
 * Uses the same successful approach as Hampshire, Herefordshire, Hertfordshire, Isle of Wight, Kent, Lancashire, Leicestershire, Lincolnshire, Merseyside, Northamptonshire, Northumberland, Nottinghamshire, Oxfordshire, Rutland, Shropshire, and Somerset
 */
...
// South Yorkshire cities with coordinates for radius searches
const SOUTH_YORKSHIRE_CITIES = [
  { name: 'Barnsley', lat: 53.5526, lng: -1.4797 },
  { name: 'Chapeltown', lat: 53.4719, lng: -1.4674 },
  { name: 'Doncaster', lat: 53.5228, lng: -1.1285 },
  { name: 'Hoyland', lat: 53.4946, lng: -1.4172 },
  { name: 'Penistone', lat: 53.5267, lng: -1.6269 },
  { name: 'Rotherham', lat: 53.4302, lng: -1.3570 },
  { name: 'Sheffield', lat: 53.3811, lng: -1.4701 },
  { name: 'Stocksbridge', lat: 53.4809, lng: -1.5956 },
  { name: 'Wombwell', lat: 53.5199, lng: -1.3950 },
  { name: 'Worsbrough', lat: 53.5411, lng: -1.4447 }
];
          ...
          if (place.name.toLowerCase().includes('pilates') ||
            ...
            try {
              ...
              if (detailsData.result) {
                ...
                // Filter for South Yorkshire addresses
                if (details.formatted_address &&
                    (details.formatted_address.includes('South Yorkshire') ||
                     details.formatted_address.includes(city.name) ||
                     isInSouthYorkshire(details.formatted_address))) {
...
function isInSouthYorkshire(address) {
  const southYorkshireIndicators = [
    'South Yorkshire',
    'S1', 'S2', 'S3', 'S4', 'S5', 'S6', 'S7', 'S8', 'S9', 'S10', 'S11', 'S12', 'S13', 'S14', 'S17', 'S18', 'S19', 'S20', 'S21', 'S25', 'S26', 'S30', 'S31', 'S32', 'S33', 'S35', 'S36', 'S60', 'S61', 'S62', 'S63', 'S64', 'S65', 'S66', 'S70', 'S71', 'S72', 'S73', 'S74', 'S75',
    'DN1', 'DN2', 'DN3', 'DN4', 'DN5', 'DN6', 'DN7', 'DN8', 'DN9', 'DN12',
    'HD8', 'HD9',
    'WF9',
    'Barnsley', 'Chapeltown', 'Doncaster', 'Hoyland', 'Penistone', 'Rotherham', 'Sheffield', 'Stocksbridge', 'Wombwell', 'Worsbrough'
  ];

  return southYorkshireIndicators.some(indicator =>
...
function determineCityFromAddress(address, searchCity) {
  ...
  // Check for specific South Yorkshire cities
  const cityNames = SOUTH_YORKSHIRE_CITIES.map(c => c.name.toLowerCase());
  for (const cityName of cityNames) {
    if (addressLower.includes(cityName)) {
      return SOUTH_YORKSHIRE_CITIES.find(c => c.name.toLowerCase() === cityName).name;
    }
  }
...
async function deleteExistingSouthYorkshireStudios() {
  console.log('🗑️ Clearing existing South Yorkshire studios...');

  const { error } = await supabase
    .from('pilates_studios')
    .delete()
    .eq('county_slug', 'south-yorkshire');

  if (error) {
    throw new Error(`Failed to delete existing studios: ${error.message}`);
  }

  console.log('✅ Cleared existing South Yorkshire studios');
}
...
async function insertCrawledStudios(studios) {
  ...
  const studiesToInsert = studios.map(studio => {
    ...
    return {
      ...
      county_slug: 'south-yorkshire',
      city_slug: citySlug,
      full_url_path: `south-yorkshire/${citySlug}/${slug}`,
      ...
      description: `Professional pilates studio in ${city}, South Yorkshire. Offering expert instruction with modern equipment and a welcoming environment for all levels.`,
  ...
  const { error: updateError } = await supabase
    ...
    .eq('slug', 'south-yorkshire')
    .eq('type', 'county');

  if (updateError) {
    console.log(`⚠️ Warning: Could not update county count: ${updateError.message}`);
  } else {
    console.log(`✅ Updated South Yorkshire county studio count to ${studiesToInsert.length}`);
  }
...
async function main() {
  console.log('🚀 Crawling Google Places API for real South Yorkshire pilates studios...\n');

  try {
    let allStudios = [];

    // Search each South Yorkshire city
    for (const city of SOUTH_YORKSHIRE_CITIES) {
    ...
    await deleteExistingSouthYorkshireStudios();
    const insertedCount = await insertCrawledStudios(uniqueStudios);

    console.log('\n' + '='.repeat(60));
    console.log('🎉 SOUTH YORKSHIRE GOOGLE PLACES CRAWL COMPLETE');
    ...
    console.log('🏆 South Yorkshire now has 100% real, Google-verified pilates studios!');
