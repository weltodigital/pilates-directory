...
/**
 * Crawl Google Places API to find real pilates studios in North Yorkshire
 * Uses the same successful approach as Hampshire, Herefordshire, Hertfordshire, Isle of Wight, Kent, Lancashire, Leicestershire, Lincolnshire, Merseyside, and Norfolk
 */
...
// North Yorkshire cities with coordinates for radius searches
const NORTH_YORKSHIRE_CITIES = [
  { name: 'Harrogate', lat: 53.9921, lng: -1.5418 },
  { name: 'Malton', lat: 54.1367, lng: -0.7983 },
  { name: 'Middlesbrough', lat: 54.5742, lng: -1.2349 },
  { name: 'Northallerton', lat: 54.3395, lng: -1.4317 },
  { name: 'Redcar', lat: 54.6161, lng: -1.0685 },
  { name: 'Ripon', lat: 54.1381, lng: -1.5238 },
  { name: 'Scarborough', lat: 54.2797, lng: -0.3986 },
  { name: 'Selby', lat: 53.7818, lng: -1.0674 },
  { name: 'Skipton', lat: 53.9616, lng: -2.0174 },
          ...
          if (place.name.toLowerCase().includes('pilates') ||
            ...
            try {
              ...
              if (detailsData.result) {
                ...
                // Filter for North Yorkshire addresses
                if (details.formatted_address &&
                    (details.formatted_address.includes('North Yorkshire') ||
                     details.formatted_address.includes(city.name) ||
                     isInNorthYorkshire(details.formatted_address))) {
...
function isInNorthYorkshire(address) {
  const northYorkshireIndicators = [
    'North Yorkshire', 'N Yorkshire', 'North Yorks',
    'YO1', 'YO7', 'YO8', 'YO11', 'YO12', 'YO13', 'YO14', 'YO17', 'YO18', 'YO19', 'YO21', 'YO22', 'YO23', 'YO24', 'YO25', 'YO26', 'YO30', 'YO31', 'YO32', 'YO41', 'YO42', 'YO43', 'YO51', 'YO60', 'YO61', 'YO62',
    'HG1', 'HG2', 'HG3', 'HG4', 'HG5',
    'DL6', 'DL7', 'DL8', 'DL9', 'DL10', 'DL11',
    'BD20', 'BD23', 'BD24',
    'LA2', 'LA6',
    'TS9', 'TS10', 'TS11', 'TS12', 'TS13', 'TS14',
    'Harrogate', 'Malton', 'Middlesbrough', 'Northallerton', 'Redcar', 'Ripon', 'Scarborough', 'Selby', 'Skipton', 'York'
  ];

  return northYorkshireIndicators.some(indicator =>
...
function determineCityFromAddress(address, searchCity) {
  ...
  // Check for specific North Yorkshire cities
  const cityNames = NORTH_YORKSHIRE_CITIES.map(c => c.name.toLowerCase());
  for (const cityName of cityNames) {
    if (addressLower.includes(cityName)) {
      return NORTH_YORKSHIRE_CITIES.find(c => c.name.toLowerCase() === cityName).name;
    }
  }
...
async function deleteExistingNorthYorkshireStudios() {
  console.log('🗑️ Clearing existing North Yorkshire studios...');

  const { error } = await supabase
    .from('pilates_studios')
    .delete()
    .eq('county_slug', 'north-yorkshire');

  if (error) {
    throw new Error(`Failed to delete existing studios: ${error.message}`);
  }

  console.log('✅ Cleared existing North Yorkshire studios');
}
...
async function insertCrawledStudios(studios) {
  ...
  const studiesToInsert = studios.map(studio => {
    ...
    return {
      ...
      county_slug: 'north-yorkshire',
      city_slug: citySlug,
      full_url_path: `north-yorkshire/${citySlug}/${slug}`,
      ...
      description: `Professional pilates studio in ${city}, North Yorkshire. Offering expert instruction with modern equipment and a welcoming environment for all levels.`,
  ...
  const { error: updateError } = await supabase
    ...
    .eq('slug', 'north-yorkshire')
    .eq('type', 'county');

  if (updateError) {
    console.log(`⚠️ Warning: Could not update county count: ${updateError.message}`);
  } else {
    console.log(`✅ Updated North Yorkshire county studio count to ${studiesToInsert.length}`);
  }
...
async function main() {
  console.log('🚀 Crawling Google Places API for real North Yorkshire pilates studios...\n');

  try {
    let allStudios = [];

    // Search each North Yorkshire city
    for (const city of NORTH_YORKSHIRE_CITIES) {
    ...
    await deleteExistingNorthYorkshireStudios();
    const insertedCount = await insertCrawledStudios(uniqueStudios);

    console.log('\n' + '='.repeat(60));
    console.log('🎉 NORTH YORKSHIRE GOOGLE PLACES CRAWL COMPLETE');
    ...
    console.log('🏆 North Yorkshire now has 100% real, Google-verified pilates studios!');
