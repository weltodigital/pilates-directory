...
/**
 * Crawl Google Places API to find real pilates studios in Staffordshire
 * Uses the same successful approach as Hampshire, Herefordshire, Hertfordshire, Isle of Wight, Kent, Lancashire, Leicestershire, Lincolnshire, Merseyside, Northamptonshire, Northumberland, Nottinghamshire, Oxfordshire, Rutland, Shropshire, Somerset, and South Yorkshire
 */
...
// Staffordshire cities with coordinates for radius searches
const STAFFORDSHIRE_CITIES = [
  { name: 'Burton upon Trent', lat: 52.8067, lng: -1.6430 },
  { name: 'Cannock', lat: 52.6906, lng: -2.0307 },
  { name: 'Kidsgrove', lat: 53.0856, lng: -2.2461 },
  { name: 'Leek', lat: 53.1036, lng: -2.0208 },
  { name: 'Lichfield', lat: 52.6833, lng: -1.8333 },
  { name: 'Newcastle-under-Lyme', lat: 53.0133, lng: -2.2281 },
  { name: 'Stafford', lat: 52.8044, lng: -2.1178 },
  { name: 'Stoke-on-Trent', lat: 53.0027, lng: -2.1794 },
  { name: 'Stone', lat: 52.9033, lng: -2.1522 },
  { name: 'Tamworth', lat: 52.6336, lng: -1.6914 }
];
          ...
          if (place.name.toLowerCase().includes('pilates') ||
            ...
            try {
              ...
              if (detailsData.result) {
                ...
                // Filter for Staffordshire addresses
                if (details.formatted_address &&
                    (details.formatted_address.includes('Staffordshire') ||
                     details.formatted_address.includes(city.name) ||
                     isInStaffordshire(details.formatted_address))) {
...
function isInStaffordshire(address) {
  const staffordshireIndicators = [
    'Staffordshire',
    'ST1', 'ST2', 'ST3', 'ST4', 'ST5', 'ST6', 'ST7', 'ST8', 'ST9', 'ST10', 'ST11', 'ST12', 'ST13', 'ST14', 'ST15', 'ST16', 'ST17', 'ST18', 'ST19', 'ST20', 'ST21',
    'WS1', 'WS2', 'WS3', 'WS8', 'WS9', 'WS10', 'WS11', 'WS12', 'WS13', 'WS14', 'WS15',
    'B77', 'B78', 'B79',
    'DE13', 'DE14', 'DE15',
    'DY3', 'DY11',
    'WV6', 'WV8', 'WV9', 'WV10', 'WV11',
    'SK9', 'SK11', 'SK17',
    'CW1', 'CW2', 'CW3', 'CW11',
    'Burton upon Trent', 'Cannock', 'Kidsgrove', 'Leek', 'Lichfield', 'Newcastle-under-Lyme', 'Stafford', 'Stoke-on-Trent', 'Stone', 'Tamworth'
  ];

  return staffordshireIndicators.some(indicator =>
...
function determineCityFromAddress(address, searchCity) {
  ...
  // Check for specific Staffordshire cities
  const cityNames = STAFFORDSHIRE_CITIES.map(c => c.name.toLowerCase());
  for (const cityName of cityNames) {
    if (addressLower.includes(cityName)) {
      return STAFFORDSHIRE_CITIES.find(c => c.name.toLowerCase() === cityName).name;
    }
  }

  // Check for variations
  if (addressLower.includes('burton upon trent') || addressLower.includes('burton on trent')) {
    return 'Burton upon Trent';
  }
  if (addressLower.includes('newcastle-under-lyme') || addressLower.includes('newcastle under lyme')) {
    return 'Newcastle-under-Lyme';
  }
  if (addressLower.includes('stoke-on-trent') || addressLower.includes('stoke on trent')) {
...
async function deleteExistingStaffordshireStudios() {
  console.log('🗑️ Clearing existing Staffordshire studios...');

  const { error } = await supabase
    .from('pilates_studios')
    .delete()
    .eq('county_slug', 'staffordshire');

  if (error) {
    throw new Error(`Failed to delete existing studios: ${error.message}`);
  }

  console.log('✅ Cleared existing Staffordshire studios');
}
...
async function insertCrawledStudios(studios) {
  ...
  const studiesToInsert = studios.map(studio => {
    ...
    return {
      ...
      city_slug: citySlug,
      full_url_path: `staffordshire/${citySlug}/${slug}`,
      ...
      description: `Professional pilates studio in ${city}, Staffordshire. Offering expert instruction with modern equipment and a welcoming environment for all levels.`,
  ...
  const { error: updateError } = await supabase
    ...
    .eq('slug', 'staffordshire')
    .eq('type', 'county');

  if (updateError) {
    console.log(`⚠️ Warning: Could not update county count: ${updateError.message}`);
  } else {
    console.log(`✅ Updated Staffordshire county studio count to ${studiesToInsert.length}`);
  }
...
async function main() {
  console.log('🚀 Crawling Google Places API for real Staffordshire pilates studios...\n');

  try {
    let allStudios = [];

    // Search each Staffordshire city
    for (const city of STAFFORDSHIRE_CITIES) {
    ...
    await deleteExistingStaffordshireStudios();
    const insertedCount = await insertCrawledStudios(uniqueStudios);

    console.log('\n' + '='.repeat(60));
    console.log('🎉 STAFFORDSHIRE GOOGLE PLACES CRAWL COMPLETE');
    ...
    console.log('🏆 Staffordshire now has 100% real, Google-verified pilates studios!');
