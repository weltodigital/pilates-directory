/**
 * Berkshire Pilates Studio Data Collection Script
 * Targets specific cities and towns within Berkshire county
 */
...
// Berkshire cities and towns
const BERKSHIRE_LOCATIONS = [
...
function extractCityFromAddress(address, location) {
  ...
  for (let i = parts.length - 2; i >= 0; i--) {
    ...
    if (BERKSHIRE_LOCATIONS.some(loc => loc.startsWith(part))) {
      return part;
    }
  }
...
async function collectStudiosForLocation(location) {
  ...
  for (const studio of studios) {
    ...
    const studioData = {
      name: details.name,
      description: `Professional pilates studio in ${city}, Berkshire offering expert instruction and quality equipment.`,
      ...
      city_slug: citySlug,
      full_url_path: `berkshire/${citySlug}/${generateSlug(details.name, city)}`,
 ...
 */
async function main() {
  console.log('🚀 Starting Berkshire Pilates Studio Collection');
  console.log('================================================\n');

  const allStudios = [];

  // Collect studios for each location
  for (const location of BERKSHIRE_LOCATIONS) {
