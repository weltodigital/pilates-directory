/**
 * Gloucestershire Pilates Studio Data Collection Script
 * Targets specific cities and towns within Gloucestershire county
 */
...
// Gloucestershire cities and towns
const GLOUCESTERSHIRE_LOCATIONS = [
...
function extractCityFromAddress(address, location) {
  ...
  for (let i = parts.length - 2; i >= 0; i--) {
    ...
    if (GLOUCESTERSHIRE_LOCATIONS.some(loc => loc.startsWith(part))) {
      return part;
    }
  }
...
async function collectStudiosForLocation(location) {
  ...
  for (const studio of studios) {
    ...
    const studioData = {
      name: details.name,
      description: `Professional pilates studio in ${city}, Gloucestershire offering expert instruction and quality equipment.`,
      ...
      city_slug: citySlug,
      full_url_path: `gloucestershire/${citySlug}/${generateSlug(details.name, city)}`,
 ...
 */
async function main() {
  console.log('🚀 Starting Gloucestershire Pilates Studio Collection');
  console.log('==================================================\n');

  const allStudios = [];

  // Collect studios for each location
  for (const location of GLOUCESTERSHIRE_LOCATIONS) {
  ...
  console.log('\n✨ Gloucestershire collection complete!');
}
