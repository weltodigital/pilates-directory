/**
 * London Pilates Studio Data Collection Script
 * Targets all 32 London boroughs plus City of London
 */
 ...
 */
function extractBoroughFromAddress(address, location) {
  // Try to extract borough from the search location first
  const locationBorough = location.split(',')[0].trim();

  // Parse the formatted address
  const parts = address.split(',');

  // Check for borough names in the address
  for (const part of parts) {
    const trimmedPart = part.trim();
    if (LONDON_BOROUGHS.some(borough => borough.startsWith(trimmedPart))) {
      return trimmedPart;
    }
  }

  // Fallback to location borough
...
/**
 * Collect pilates studios for a specific borough
 */
async function collectStudiosForBorough(location) {
  ...
  for (const studio of studios) {
    ...
    const borough = extractBoroughFromAddress(details.formatted_address, location);
    const boroughSlug = borough.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '');
    ...
    const studioData = {
      name: details.name,
      description: `Professional pilates studio in ${borough}, London offering expert instruction and quality equipment.`,
      ...
      county_slug: 'london',
      city_slug: boroughSlug,
      full_url_path: `london/${boroughSlug}/${generateSlug(details.name, borough)}`,
 ...
 */
async function main() {
  console.log('🚀 Starting London Pilates Studio Collection');
  console.log('=============================================\n');

  const allStudios = [];

  // Collect studios for each borough
  for (const location of LONDON_BOROUGHS) {
    const studios = await collectStudiosForBorough(location);
...
module.exports = { collectStudiosForBorough };