/**
 * Cornwall Pilates Studio Data Collection Script
 * Targets specific cities and towns within Cornwall county
 */
...
// Cornwall cities and towns
const CORNWALL_LOCATIONS = [
  'Bodmin, Cornwall, UK',
...
function extractCityFromAddress(address, location) {
  ...
  for (let i = parts.length - 2; i >= 0; i--) {
    ...
    if (CORNWALL_LOCATIONS.some(loc => loc.startsWith(part))) {
      return part;
    }
  }
...
async function collectStudiosForLocation(location) {
  ...
  for (const studio of studios) {
    ...
    const studioData = {
      name: details.name,
      description: `Professional pilates studio in ${city}, Cornwall offering expert instruction and quality equipment.`,
      ...
      city_slug: citySlug,
      full_url_path: `cornwall/${citySlug}/${generateSlug(details.name, city)}`,
 ...
 */
async function main() {
  console.log('🚀 Starting Cornwall Pilates Studio Collection');
  console.log('===========================================\n');

  const allStudios = [];

  // Collect studios for each location
  for (const location of CORNWALL_LOCATIONS) {
