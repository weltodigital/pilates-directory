/**
 * Cheshire Pilates Studio Data Collection Script
 * Targets specific cities and towns within Cheshire county
 */
...
// Cheshire cities and towns
const CHESHIRE_LOCATIONS = [
  'Chester, Cheshire, UK',
  'Crewe, Cheshire, UK',
...
function extractCityFromAddress(address, location) {
  ...
  for (let i = parts.length - 2; i >= 0; i--) {
    ...
    if (CHESHIRE_LOCATIONS.some(loc => loc.startsWith(part))) {
      return part;
    }
  }
...
async function collectStudiosForLocation(location) {
  ...
  for (const studio of studios) {
    ...
    const studioData = {
      name: details.name,
      description: `Professional pilates studio in ${city}, Cheshire offering expert instruction and quality equipment.`,
      ...
      county_slug: 'cheshire',
      city_slug: citySlug,
      full_url_path: `cheshire/${citySlug}/${generateSlug(details.name, city)}`,
 ...
 */
async function main() {
  console.log('🚀 Starting Cheshire Pilates Studio Collection');
  console.log('============================================\n');

  const allStudios = [];

  // Collect studios for each location
  for (const location of CHESHIRE_LOCATIONS) {
