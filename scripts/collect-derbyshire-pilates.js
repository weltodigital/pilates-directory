/**
 * Derbyshire Pilates Studio Data Collection Script
 * Targets specific cities and towns within Derbyshire county
 */
...
// Derbyshire cities and towns
const DERBYSHIRE_LOCATIONS = [
...
function extractCityFromAddress(address, location) {
  ...
  for (let i = parts.length - 2; i >= 0; i--) {
    ...
    if (DERBYSHIRE_LOCATIONS.some(loc => loc.startsWith(part))) {
      return part;
    }
  }
...
async function collectStudiosForLocation(location) {
  ...
  for (const studio of studios) {
    ...
    const studioData = {
      name: details.name,
      description: `Professional pilates studio in ${city}, Derbyshire offering expert instruction and quality equipment.`,
      ...
      county_slug: 'derbyshire',
      city_slug: citySlug,
      full_url_path: `derbyshire/${citySlug}/${generateSlug(details.name, city)}`,
 ...
 */
async function main() {
  console.log('🚀 Starting Derbyshire Pilates Studio Collection');
  console.log('============================================\n');

  const allStudios = [];

  // Collect studios for each location
  for (const location of DERBYSHIRE_LOCATIONS) {
  ...
  console.log('\n✨ Derbyshire collection complete!');
}
