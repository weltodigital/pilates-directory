/**
 * Fix Wokingham Studios - Move misplaced studios to correct cities
 * Based on actual addresses in the database
 */
...
const STUDIO_MOVES = [
  {
    ...
    currentCity: 'Wokingham',
    newCity: 'Bracknell',
    reason: 'Address shows Bracknell location'
  },
  {
    name: 'Live Well Pilates',
    currentCity: 'Wokingham',
    newCity: 'Bracknell',
    reason: 'Address shows Bracknell location'
  },
  ...
  {
    name: "Zo's Pilates & Nutrition",
    currentCity: 'Wokingham',
    newCity: 'Reading',
    reason: 'Address shows Reading location'
  }
];
...
async function moveStudio(studioMove) {
  ...
  try {
    ...
    console.log(`   📍 Found studio: ${studio.name} at ${studio.address}`);

    // Calculate new slug values
    const newCitySlug = studioMove.newCity.toLowerCase().replace(/\s+/g, '-');
    const newFullUrlPath = `berkshire/${newCitySlug}/${generateSlug(studio.name, studioMove.newCity)}`;
...
async function main() {
  console.log('🚀 Starting Wokingham Studio Location Fix');
  console.log('========================================\n');
  ...
  if (successCount > 0) {
    ...
    console.log('The studios have been moved to their correct cities based on their addresses.');
  }
}
