/**
 * Devon Pilates Studio Data Collection Script
 * Targets specific cities and towns within Devon county
 */
...
function extractCityFromAddress(address, location) {
  ...
  for (let i = parts.length - 2; i >= 0; i--) {
    ...
    if (DEVON_LOCATIONS.some(loc => loc.startsWith(part))) {
      return part;
    }
  }
...
async function collectStudiosForLocation(location) {
  ...
  for (const studio of studios) {
    ...
    const studioData = {
      name: details.name,
      description: `Professional pilates studio in ${city}, Devon offering expert instruction and quality equipment.`,
      ...
      county_slug: 'devon',
      city_slug: citySlug,
      full_url_path: `devon/${citySlug}/${generateSlug(details.name, city)}`,
 ...
 */
async function main() {
  console.log('🚀 Starting Devon Pilates Studio Collection');
  console.log('========================================\n');

  const allStudios = [];

  // Collect studios for each location
  for (const location of DEVON_LOCATIONS) {
