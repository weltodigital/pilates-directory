/**
 * Cumbria Pilates Studio Data Collection Script
 * Targets specific cities and towns within Cumbria county
 */
...
// Cumbria cities and towns
const CUMBRIA_LOCATIONS = [
...
function extractCityFromAddress(address, location) {
  ...
  for (let i = parts.length - 2; i >= 0; i--) {
    ...
    if (CUMBRIA_LOCATIONS.some(loc => loc.startsWith(part))) {
      return part;
    }
  }
...
async function collectStudiosForLocation(location) {
  ...
  for (const studio of studios) {
    ...
    const studioData = {
      name: details.name,
      description: `Professional pilates studio in ${city}, Cumbria offering expert instruction and quality equipment.`,
      ...
      county_slug: 'cumbria',
      city_slug: citySlug,
      full_url_path: `cumbria/${citySlug}/${generateSlug(details.name, city)}`,
 ...
 */
async function main() {
  console.log('🚀 Starting Cumbria Pilates Studio Collection');
  console.log('==========================================\n');

  const allStudios = [];

  // Collect studios for each location
  for (const location of CUMBRIA_LOCATIONS) {
