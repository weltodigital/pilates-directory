/**
 * Durham Pilates Studio Data Collection Script
 * Targets specific cities and towns within Durham county
 */
...
// Durham cities and towns
const DURHAM_LOCATIONS = [
...
function extractCityFromAddress(address, location) {
  ...
  for (let i = parts.length - 2; i >= 0; i--) {
    ...
    if (DURHAM_LOCATIONS.some(loc => loc.startsWith(part))) {
      return part;
    }
  }
...
async function collectStudiosForLocation(location) {
  ...
  for (const studio of studios) {
    ...
    const studioData = {
      name: details.name,
      description: `Professional pilates studio in ${city}, Durham offering expert instruction and quality equipment.`,
      ...
      county_slug: 'durham',
      city_slug: citySlug,
      full_url_path: `durham/${citySlug}/${generateSlug(details.name, city)}`,
 ...
 */
async function main() {
  console.log('🚀 Starting Durham Pilates Studio Collection');
  console.log('=========================================\n');

  const allStudios = [];

  // Collect studios for each location
  for (const location of DURHAM_LOCATIONS) {
