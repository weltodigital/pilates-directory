/**
 * Essex Pilates Studio Data Collection Script
 * Targets specific cities and towns within Essex county
 */
...
function extractCityFromAddress(address, location) {
  ...
  for (let i = parts.length - 2; i >= 0; i--) {
    ...
    if (ESSEX_LOCATIONS.some(loc => loc.startsWith(part))) {
      return part;
    }
  }
...
async function collectStudiosForLocation(location) {
  ...
  for (const studio of studios) {
    ...
    const studioData = {
      name: details.name,
      description: `Professional pilates studio in ${city}, Essex offering expert instruction and quality equipment.`,
      ...
      county_slug: 'essex',
      city_slug: citySlug,
      full_url_path: `essex/${citySlug}/${generateSlug(details.name, city)}`,
 ...
 */
async function main() {
  console.log('🚀 Starting Essex Pilates Studio Collection');
  console.log('==========================================\n');

  const allStudios = [];

  // Collect studios for each location
  for (const location of ESSEX_LOCATIONS) {
