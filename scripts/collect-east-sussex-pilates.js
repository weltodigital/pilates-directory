/**
 * East Sussex Pilates Studio Data Collection Script
 * Targets specific cities and towns within East Sussex county
 */
...
// East Sussex cities and towns
const EAST_SUSSEX_LOCATIONS = [
...
function extractCityFromAddress(address, location) {
  ...
  for (let i = parts.length - 2; i >= 0; i--) {
    ...
    if (EAST_SUSSEX_LOCATIONS.some(loc => loc.startsWith(part))) {
      return part;
    }
  }
...
async function collectStudiosForLocation(location) {
  ...
  for (const studio of studios) {
    ...
    const studioData = {
      name: details.name,
      description: `Professional pilates studio in ${city}, East Sussex offering expert instruction and quality equipment.`,
      ...
      city_slug: citySlug,
      full_url_path: `east-sussex/${citySlug}/${generateSlug(details.name, city)}`,
 ...
 */
async function main() {
  console.log('🚀 Starting East Sussex Pilates Studio Collection');
  console.log('===============================================\n');

  const allStudios = [];

  // Collect studios for each location
  for (const location of EAST_SUSSEX_LOCATIONS) {
  ...
  console.log('\n✨ East Sussex collection complete!');
}
