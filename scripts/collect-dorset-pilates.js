/**
 * Dorset Pilates Studio Data Collection Script
 * Targets specific cities and towns within Dorset county
 */
...
// Dorset cities and towns
const DORSET_LOCATIONS = [
...
function extractCityFromAddress(address, location) {
  ...
  for (let i = parts.length - 2; i >= 0; i--) {
    ...
    if (DORSET_LOCATIONS.some(loc => loc.startsWith(part))) {
      return part;
    }
  }
...
async function collectStudiosForLocation(location) {
  ...
  for (const studio of studios) {
    ...
    const studioData = {
      name: details.name,
      description: `Professional pilates studio in ${city}, Dorset offering expert instruction and quality equipment.`,
      ...
      county_slug: 'dorset',
      city_slug: citySlug,
      full_url_path: `dorset/${citySlug}/${generateSlug(details.name, city)}`,
 ...
 */
async function main() {
  console.log('🚀 Starting Dorset Pilates Studio Collection');
  console.log('=========================================\n');

  const allStudios = [];

  // Collect studios for each location
  for (const location of DORSET_LOCATIONS) {
