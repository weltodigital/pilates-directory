/**
 * Cambridgeshire Pilates Studio Data Collection Script
 * Targets specific cities and towns within Cambridgeshire county
 */
...
// Cambridgeshire cities and towns
const CAMBRIDGESHIRE_LOCATIONS = [
...
function extractCityFromAddress(address, location) {
  ...
  for (let i = parts.length - 2; i >= 0; i--) {
    ...
    if (CAMBRIDGESHIRE_LOCATIONS.some(loc => loc.startsWith(part))) {
      return part;
    }
  }
...
async function collectStudiosForLocation(location) {
  ...
  for (const studio of studios) {
    ...
    const studioData = {
      name: details.name,
      description: `Professional pilates studio in ${city}, Cambridgeshire offering expert instruction and quality equipment.`,
      ...
      city_slug: citySlug,
      full_url_path: `cambridgeshire/${citySlug}/${generateSlug(details.name, city)}`,
 ...
 */
async function main() {
  console.log('🚀 Starting Cambridgeshire Pilates Studio Collection');
  console.log('=================================================\n');

  const allStudios = [];

  // Collect studios for each location
  for (const location of CAMBRIDGESHIRE_LOCATIONS) {
  ...
  console.log('\n✨ Cambridgeshire collection complete!');
}
