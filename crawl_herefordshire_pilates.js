...
/**
 * Crawl Google Places API to find real pilates studios in Herefordshire
 * Uses the same successful approach as Hampshire
 */
...
// Herefordshire cities with coordinates for radius searches
const HEREFORDSHIRE_CITIES = [
  { name: 'Bromyard', lat: 52.1879, lng: -2.5159 },
  { name: 'Eardisley', lat: 52.1269, lng: -3.1139 },
  { name: 'Hay-on-Wye', lat: 52.0739, lng: -3.1289 },
  { name: 'Hereford', lat: 52.0567, lng: -2.7156 },
  { name: 'Kington', lat: 52.2067, lng: -3.0674 },
  { name: 'Ledbury', lat: 52.0364, lng: -2.4264 },
  { name: 'Leominster', lat: 52.2253, lng: -2.7387 },
  { name: 'Pembridge', lat: 52.1876, lng: -2.9218 },
  { name: 'Ross-on-Wye', lat: 51.9131, lng: -2.5831 },
  { name: 'Weobley', lat: 52.1326, lng: -2.8731 }
];
...
async function searchPilatesStudiosInCity(city) {
  ...
  const radius = 15000; // 15km radius for rural areas
          ...
          if (place.name.toLowerCase().includes('pilates') ||
            ...
            try {
              ...
              if (detailsData.result) {
                ...
                // Filter for Herefordshire addresses
                if (details.formatted_address &&
                    (details.formatted_address.includes('Herefordshire') ||
                     details.formatted_address.includes(city.name) ||
                     isInHerefordshire(details.formatted_address))) {
...
function isInHerefordshire(address) {
  const herefordshireIndicators = [
    'Herefordshire', 'Hereford', 'HR1', 'HR2', 'HR3', 'HR4', 'HR5', 'HR6', 'HR7', 'HR8', 'HR9',
    'Bromyard', 'Eardisley', 'Hay-on-Wye', 'Kington', 'Ledbury', 'Leominster', 'Pembridge', 'Ross-on-Wye', 'Weobley'
  ];

  return herefordshireIndicators.some(indicator =>
...
function determineCityFromAddress(address, searchCity) {
  ...
  // Check for specific Herefordshire cities
  const cityNames = HEREFORDSHIRE_CITIES.map(c => c.name.toLowerCase());
  for (const cityName of cityNames) {
    if (addressLower.includes(cityName)) {
      return HEREFORDSHIRE_CITIES.find(c => c.name.toLowerCase() === cityName).name;
    }
  }
...
async function deleteExistingHerefordshireStudios() {
  console.log('🗑️ Clearing existing Herefordshire studios...');

  const { error } = await supabase
    .from('pilates_studios')
    .delete()
    .eq('county_slug', 'herefordshire');

  if (error) {
    throw new Error(`Failed to delete existing studios: ${error.message}`);
  }

  console.log('✅ Cleared existing Herefordshire studios');
}
...
async function insertCrawledStudios(studios) {
  ...
  const studiesToInsert = studios.map(studio => {
    ...
    return {
      ...
      county_slug: 'herefordshire',
      city_slug: citySlug,
      full_url_path: `herefordshire/${citySlug}/${slug}`,
      ...
      description: `Professional pilates studio in ${city}, Herefordshire. Offering expert instruction with modern equipment and a welcoming environment for all levels.`,
  ...
  const { error: updateError } = await supabase
    ...
    .eq('slug', 'herefordshire')
    .eq('type', 'county');

  if (updateError) {
    console.log(`⚠️ Warning: Could not update county count: ${updateError.message}`);
  } else {
    console.log(`✅ Updated Herefordshire county studio count to ${studiesToInsert.length}`);
  }
...
async function main() {
  console.log('🚀 Crawling Google Places API for real Herefordshire pilates studios...\n');

  try {
    let allStudios = [];

    // Search each Herefordshire city
    for (const city of HEREFORDSHIRE_CITIES) {
    ...
    await deleteExistingHerefordshireStudios();
    const insertedCount = await insertCrawledStudios(uniqueStudios);

    console.log('\n' + '='.repeat(60));
    console.log('🎉 HEREFORDSHIRE GOOGLE PLACES CRAWL COMPLETE');
    ...
    console.log('🏆 Herefordshire now has 100% real, Google-verified pilates studios!');
