...
// Rutland cities with coordinates for radius searches
const RUTLAND_CITIES = [
  { name: 'Cottesmore', lat: 52.7335, lng: -0.6300 },
  { name: 'Greetham', lat: 52.7639, lng: -0.6456 },
  { name: 'Ketton', lat: 52.6113, lng: -0.5244 },
  { name: 'Langham', lat: 52.6719, lng: -0.5889 },
  { name: 'Market Overton', lat: 52.7400, lng: -0.5617 },
  { name: 'Oakham', lat: 52.6703, lng: -0.7344 },
  { name: 'Ryhall', lat: 52.6569, lng: -0.4761 },
  { name: 'Tinwell', lat: 52.6311, lng: -0.4878 },
  { name: 'Uppingham', lat: 52.5880, lng: -0.7210 },
  { name: 'Whissendine', lat: 52.7241, lng: -0.6872 }
];
          ...
          if (place.name.toLowerCase().includes('pilates') ||
            ...
            try {
              ...
              if (detailsData.result) {
                ...
                // Filter for Rutland addresses
                if (details.formatted_address &&
                    (details.formatted_address.includes('Rutland') ||
                     details.formatted_address.includes(city.name) ||
                     isInRutland(details.formatted_address))) {
...
function isInRutland(address) {
  const rutlandIndicators = [
    'Rutland',
    'LE15',
    'PE9',
    'Cottesmore', 'Greetham', 'Ketton', 'Langham', 'Market Overton', 'Oakham', 'Ryhall', 'Tinwell', 'Uppingham', 'Whissendine'
  ];

  return rutlandIndicators.some(indicator =>
...
function determineCityFromAddress(address, searchCity) {
  ...
  // Check for specific Rutland cities
  const cityNames = RUTLAND_CITIES.map(c => c.name.toLowerCase());
  for (const cityName of cityNames) {
    if (addressLower.includes(cityName)) {
      return RUTLAND_CITIES.find(c => c.name.toLowerCase() === cityName).name;
    }
  }

  // Check for variations
  if (addressLower.includes('market overton')) {
    return 'Market Overton';
  }
...
async function deleteExistingRutlandStudios() {
  console.log('🗑️ Clearing existing Rutland studios...');

  const { error } = await supabase
    .from('pilates_studios')
    .delete()
    .eq('county_slug', 'rutland');

  if (error) {
    throw new Error(`Failed to delete existing studios: ${error.message}`);
  }

  console.log('✅ Cleared existing Rutland studios');
}
...
async function insertCrawledStudios(studios) {
  ...
  const studiesToInsert = studios.map(studio => {
    ...
    return {
      ...
      county_slug: 'rutland',
      city_slug: citySlug,
      full_url_path: `rutland/${citySlug}/${slug}`,
  ...
  if (updateError) {
    ...
    console.log(`✅ Updated Rutland county studio count to ${studiesToInsert.length}`);
  }
...
async function main() {
  console.log('🚀 Crawling Google Places API for real Rutland pilates studios...\n');

  try {
    let allStudios = [];

    // Search each Rutland city
    for (const city of RUTLAND_CITIES) {
    ...
    await deleteExistingRutlandStudios();
    const insertedCount = await insertCrawledStudios(uniqueStudios);

    console.log('\n' + '='.repeat(60));
    console.log('🎉 RUTLAND GOOGLE PLACES CRAWL COMPLETE');
    ...
    console.log('🏆 Rutland now has 100% real, Google-verified pilates studios!');
