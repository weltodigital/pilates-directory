...
/**
 * Crawl Google Places API to find real pilates studios in Hampshire
 * Uses proper "pilates" category searches to find legitimate businesses
 */
...
// Hampshire cities with coordinates for radius searches
const HAMPSHIRE_CITIES = [
  { name: 'Winchester', lat: 51.0632, lng: -1.3080 },
  { name: 'Southampton', lat: 50.9097, lng: -1.4044 },
  { name: 'Portsmouth', lat: 50.8197, lng: -1.0880 },
  { name: 'Basingstoke', lat: 51.2664, lng: -1.0873 },
  { name: 'Andover', lat: 51.2082, lng: -1.4778 },
  { name: 'Eastleigh', lat: 50.9697, lng: -1.3498 },
  { name: 'Fareham', lat: 50.8555, lng: -1.1865 },
  { name: 'Gosport', lat: 50.7949, lng: -1.1294 },
  { name: 'Havant', lat: 50.8551, lng: -0.9805 },
  { name: 'Alton', lat: 51.1528, lng: -0.9731 }
];
...
async function searchPilatesStudiosInCity(city) {
  ...
  const radius = 10000; // 10km radius - reduced for faster processing
  const keywords = ['pilates', 'pilates studio']; // Reduced keywords for speed
  ...
  for (const keyword of keywords) {
    try {
          ...
          if (place.name.toLowerCase().includes('pilates') ||
            ...
            try {
              ...
              if (detailsData.result) {
                ...
                // Filter for Hampshire addresses
                if (details.formatted_address &&
                    (details.formatted_address.includes('Hampshire') ||
                     details.formatted_address.includes(city.name) ||
                     isInHampshire(details.formatted_address))) {
              ...
              // Rate limiting for details API - reduced for speed
      ...
      // Rate limiting between searches - reduced for speed
...
function isInHampshire(address) {
  const hampshireIndicators = [
    'Hampshire', 'Hants', 'SO14', 'SO15', 'SO16', 'SO17', 'SO18', 'SO19', 'SO20', 'SO21', 'SO22', 'SO23', 'SO24',
    'PO1', 'PO2', 'PO3', 'PO4', 'PO5', 'PO6', 'PO7', 'PO8', 'PO9', 'PO10', 'PO11', 'PO12', 'PO13', 'PO14', 'PO15', 'PO16',
    'RG21', 'RG22', 'RG23', 'RG24', 'RG25', 'RG26', 'RG27', 'RG28', 'RG29',
    'SP10', 'SP11', 'GU34', 'GU35'
  ];

  return hampshireIndicators.some(indicator =>
...
function determineCityFromAddress(address, searchCity) {
  ...
  // Check for specific Hampshire cities
  const cityNames = HAMPSHIRE_CITIES.map(c => c.name.toLowerCase());
  for (const cityName of cityNames) {
    if (addressLower.includes(cityName)) {
      return HAMPSHIRE_CITIES.find(c => c.name.toLowerCase() === cityName).name;
    }
  }

  // Check for Southsea (part of Portsmouth)
  if (addressLower.includes('southsea')) {
    return 'Portsmouth';
  }
...
async function insertCrawledStudios(studios) {
  ...
  const studiesToInsert = studios.map(studio => {
    ...
    return {
      ...
      description: `Professional pilates studio in ${city}, Hampshire. Offering expert instruction with modern equipment and a welcoming environment for all levels.`,
...
async function main() {
  console.log('🚀 Crawling Google Places API for real Hampshire pilates studios...\n');

  try {
    let allStudios = [];

    // Search each Hampshire city
    for (const city of HAMPSHIRE_CITIES) {
      ...
      // Rate limiting between cities - reduced for speed
    ...
    console.log('\n' + '='.repeat(60));
    console.log('🎉 GOOGLE PLACES CRAWL COMPLETE');
