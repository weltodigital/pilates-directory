-- Comprehensive Easy Recipe Categories
-- Organized by type: Food Type, Cuisine, Diet, Meal Type, Cooking Method, Occasion

-- Clear existing categories first (optional - comment out if you want to keep current ones)
...
-- INTERNATIONAL TAPAS & SMALL PLATES
...
CREATE INDEX IF NOT EXISTS idx_categories_featured_name ON categories(featured, name);
CREATE INDEX IF NOT EXISTS idx_categories_slug_featured ON categories(slug, featured);